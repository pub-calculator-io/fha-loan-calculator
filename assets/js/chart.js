...
import("./assets/js/lib/chartjs/chart.js").then((e) => {
	...
	let data = [
		{
			data: [34, 36, 16, 7, 2, 5],
			labels: ['34%', '36%', '16%', '7%', '2%', '5%'],
			backgroundColor: [colors[theme].yellow, colors[theme].purple, colors[theme].sky, colors[theme].red, colors[theme].green, colors[theme].pink],
			borderColor: '#DDD6FE',
			borderWidth: 0,
		},
	];
	...
	let switchThemeDonut = function(theme) {
		...
		donutBig.data.datasets[0].backgroundColor = [colors[theme].yellow, colors[theme].purple, colors[theme].sky, colors[theme].red, colors[theme].green, colors[theme].pink];
		donutBig.update()
	}
